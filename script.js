...
function secondsToMinutesSeconds(seconds) {
  ...
  const formattedSeconeds = String(remainingSeconds).padStart(2, "0");

  return `${formattedMinutes}: ${formattedSeconeds}`;
}

async function getSongs(folder) {
  currfolder = folder;
  let a = await fetch(`http://127.0.0.1:5601/${folder}/`);
...
async function displayAlbums() {
  let a = await fetch("http://127.0.0.1:5601/songs/");
  let response = await a.text();
  let div = document.createElement("div");
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (e.href.includes("/songs/")) {
      ...
      let a = await fetch(`http://127.0.0.1:5601/songs/${folder}/info.json`);
...
async function main() {
  ...
  //Attach an event listener to play, next an
  ...
  document.querySelector(".seekbar").addEventListener("click", (e) => {
    ...
    document.querySelector(".circle").style.left = "green" + parcent + "%";
